...
describe('Save As names the download', () => {
  it('writes the typed name and adopts it as the document title', async () => {
    const download = await import('@/ui/download');
    const spy = vi.spyOn(download, 'downloadBlob').mockImplementation(() => {});
    addCustomPanel('shelf');

    await saveToFile('Kitchen Remodel');

    expect(spy.mock.calls[0]?.[1]).toBe('Kitchen Remodel.forma.json');
    expect(doc().docTitle).toBe('Kitchen Remodel');
    spy.mockRestore();
  });

  it('sanitizes characters a filesystem would reject', async () => {
    const download = await import('@/ui/download');
    const spy = vi.spyOn(download, 'downloadBlob').mockImplementation(() => {});
    addCustomPanel('shelf');

    await saveToFile('Kitchen/v2:final?');

    expect(spy.mock.calls[0]?.[1]).toBe('Kitchen-v2-final-.forma.json');
    // Runs of rejected characters collapse to one dash; spacing is left alone.
    expect(sanitizeFilename('Kitchen<>v2')).toBe('Kitchen-v2');
    expect(sanitizeFilename('Kitchen: v2')).toBe('Kitchen- v2');
    spy.mockRestore();
  });

  it('falls back to the current title when no name is given', async () => {
    const download = await import('@/ui/download');
    const spy = vi.spyOn(download, 'downloadBlob').mockImplementation(() => {});
    addCustomPanel('shelf');
    renameDocument('Hallway Unit');

    await saveToFile();

    expect(spy.mock.calls[0]?.[1]).toBe('Hallway Unit.forma.json');
    spy.mockRestore();
  });

  it('keeps a blank name from erasing the title', async () => {
    const download = await import('@/ui/download');
    const spy = vi.spyOn(download, 'downloadBlob').mockImplementation(() => {});
    addCustomPanel('shelf');
    renameDocument('Hallway Unit');
    ...
    expect(spy.mock.calls[0]?.[1]).toBe('Untitled Design.forma.json');
    spy.mockRestore();
  });
});

describe('saving through the file picker', () => {
  type Picker = (options?: { suggestedName?: string }) => Promise<FileSystemFileHandle>;
  const g = globalThis as { showSaveFilePicker?: Picker };

  /** A handle whose writes are recorded, so overwrite behaviour is observable. */
  function fakeHandle(name: string, opts: { failWrite?: boolean } = {}) {
    ...
    const handle = {
      ...
      createWritable: async () => {
        if (opts.failWrite) throw new DOMException('no', 'NotAllowedError');
        return {
    ...
    return handle as unknown as FileSystemFileHandle & { writes: string[] };
  }

  function installPicker(impl: Picker) {
    g.showSaveFilePicker = impl;
  }

  beforeEach(() => {
    delete g.showSaveFilePicker;
    // The retained handle is module state that outlives a store hydrate, so it
    // has to be released between tests the same way the app releases it.
    newDocument();
  });

  it('writes through the picked handle instead of downloading', async () => {
    const download = await import('@/ui/download');
    const spy = vi.spyOn(download, 'downloadBlob').mockImplementation(() => {});
    const handle = fakeHandle('Kitchen.forma.json');
    installPicker(async () => handle);
    addCustomPanel('shelf');

    await saveToFile('Kitchen');

    expect(handle.writes).toHaveLength(1);
    expect(spy).not.toHaveBeenCalled();
    // The name chosen on disk becomes the document title.
    expect(doc().docTitle).toBe('Kitchen');
    spy.mockRestore();
  });

  it('overwrites the same file on later saves without asking again', async () => {
    const handle = fakeHandle('Kitchen.forma.json');
    let prompts = 0;
    installPicker(async () => {
      prompts += 1;
  ...
  it('re-picks after a rename, so Save never writes the stale filename', async () => {
    const first = fakeHandle('Kitchen.forma.json');
    const second = fakeHandle('Hallway.forma.json');
    const handles = [first, second];
    installPicker(async () => handles.shift()!);
    addCustomPanel('shelf');

    await saveToFile('Kitchen');
    renameDocument('Hallway');
    await saveToFile();

    expect(first.writes).toHaveLength(1);
    expect(second.writes).toHaveLength(1);
    expect(doc().docTitle).toBe('Hallway');
  });

  it('falls back to a download when the write fails after the dialog (BUG-026)', async () => {
    const download = await import('@/ui/download');
    const spy = vi.spyOn(download, 'downloadBlob').mockImplementation(() => {});
    installPicker(async () => fakeHandle('Kitchen.forma.json', { failWrite: true }));
    addCustomPanel('shelf');

    await expect(saveToFile('Kitchen')).resolves.toBe(true);

    expect(spy.mock.calls[0]?.[1]).toBe('Kitchen.forma.json');
    spy.mockRestore();
  });

  it('falls back to a download when the dialog never appears (BUG-026)', async () => {
    const download = await import('@/ui/download');
    const spy = vi.spyOn(download, 'downloadBlob').mockImplementation(() => {});
    // Instant AbortError: headless Chromium and some webviews reject without
    // ever showing anything, which no person could have dismissed that fast.
    installPicker(async () => {
  ...
  it('respects a real cancel and writes nothing', async () => {
    const download = await import('@/ui/download');
    const spy = vi.spyOn(download, 'downloadBlob').mockImplementation(() => {});
    installPicker(async () => {
    ...
    await expect(saveToFile('Kitchen')).resolves.toBe(false);

    expect(spy).not.toHaveBeenCalled();
  });

  it('downloads when the browser has no picker at all', async () => {
    const download = await import('@/ui/download');
    const spy = vi.spyOn(download, 'downloadBlob').mockImplementation(() => {});
    addCustomPanel('shelf');
  ...
  it('drops the handle when a new design starts', async () => {
    const handle = fakeHandle('Kitchen.forma.json');
    let prompts = 0;
    installPicker(async () => {
      prompts += 1;
