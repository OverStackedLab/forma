...
- Group header: `height:30px`, `padding:0 8px`, `border-radius:6px`. A 14px drag handle sits left of a 16px `▾`/`▸` chevron when more than one group exists; label 12px/600 `rgba(238,233,226,.85)`; count right-aligned, 10.5px IBM Plex Mono `rgba(238,233,226,.35)`. Drag the handle to reorder groups. Shift-click (or ⌘/Ctrl-click) the row adds or removes the whole group without replacing the rest of the selection. The row highlight stays on when the group is fully included in a larger multi-select. Clicking the name still selects only that group.
...
- **Panels** — 2-up grid of insertable parts: Shelf, Side Panel, Back Panel, Divider. **Fronts** — solid Door plus AXSTAD-style glass doors (400/600 × 800/1000, 19 mm, 78 mm shaker frame). **BODBYN** — off-white IKEA fronts: solid doors, drawer fronts, and the 40×40 glass door with a cross-rail, 19 mm, 70 mm frame, bevelled recessed panel or inset glass. **Drawers** — four-piece boxes (18 mm sides, 8 mm bottom and back) sized to matching METOD openings; decorative fronts stay separate. **Hardware** — Knob, BAGGANÄS, ENERYDA bow pull, BORGHAMN square bar (170 mm), ENHET leg.
...
- **Measure banner** — top-center when measure mode is on: `background:rgba(26,24,21,.85)`, `color:#EEE9E2`, 11.5px, `padding:6px 14px`, `border-radius:20px`. Copy: "Click two points · Shift for a straight axis · Front/Side/Top stay in the view plane".
- **Measure label** — follows the midpoint of the measured segment, `transform:translate(-50%,-130%)`, `background:#1A1815`, `color:#4FA3FF`, IBM Plex Mono 11px, `border:1px solid rgba(79,163,255,.4)`, `border-radius:5px`, `pointer-events:none`. Content: `{n} {unit}`; a diagonal span also shows ΔX / ΔY / ΔZ.
- **Selection dimensions** — one selected piece or group draws witness lines and a length label in `#4FA3FF` to the nearest facing neighbour in each direction. With the move gizmo on, flush faces and a moving group lining up with another group use `#C68A46`. A cabinet or fully selected group also shows overall W/H/D. With the scale gizmo on, click an overall label to type a size. Two selected units lock that pair. Click a clearance label to type a gap; the gizmo's parts move to match. Hidden in Render.
...
- **Shift-click** a part → add/toggle that part. Shift-click (or ⌘/Ctrl-click) a group row adds or removes every member.
- **One selected unit** draws clearance to the nearest facing neighbour in each direction (above/below, left/right, front/back), so a shelf can be moved on its own. With the move gizmo on, flush faces draw the same witnesses in `#C68A46`, and a selected group also shows when its edges line up with another group. A cabinet (or fully selected group) also draws overall W/H/D. A fully selected group is one body and does not dimension to its own members. Click a clearance label to type the gap; the selected unit moves. With the scale gizmo on, overall labels (including a single part's W/H/D) can be typed to resize.
...
### Measuring
Toggle in the toolbar. Click two points on the model; each click raycasts, then snaps to a nearby vertex or edge when object snap is on. After the first click, a rubber-band preview follows the cursor. Front / Side / Top copy the camera look-axis from the first point so the span stays in the view plane. Hold Shift to lock the second point onto the dominant world axis (X, Y or Z). Renders 8 mm spheres and a dashed line in `#4FA3FF`, with a DOM label at the projected midpoint showing the distance in the current display unit; a diagonal span also lists ΔX / ΔY / ΔZ. A third click starts a fresh measurement.

Selecting a piece or group draws SketchUp-style witnesses to the nearest facing neighbour in each direction. With the move gizmo on, flush faces use the brass accent (`#C68A46`) instead of selection blue, and a moving group also shows 0 mm when its min or max lines up with another group (Align Left / Tops style). Alignment lines span the shared face or the gap between the two boxes so they stay visible after a snap, instead of collapsing to a point. A cabinet — or any fully selected group — also draws overall width, height, and depth on the carcass. Selecting exactly two pieces or groups locks that pair instead. Click a clearance label to type the gap in the current display unit; the same parts the gizmo would drive move to match. With the scale gizmo on, overall labels can be typed to resize (a single part, a rigid group, or a cabinet). Overlapping interiors stay quiet. The overlay reads live mesh bounds so a gizmo drag stays truthful, and it hides in Render.
